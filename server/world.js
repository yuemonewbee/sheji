...
// 最后存活（歼灭赛）：回合制、不复活
const LASTMAN_ROUNDS = 3;        // 先赢 3 回合获胜（可调）
const ROUND_OVER_TICKS = 120;    // 回合结束后展示 ~4 秒，再开下一回合
...
function createWorld(spec) {
  ...
  const world = {
    ...
    mapName: map.name,
    mode: ['gungame', 'koth', 'ctf', 'lastman'].indexOf(spec.mode) >= 0 ? spec.mode : 'tdm', // 游戏模式
  ...
  // 最后存活：回合制、不复活。teamScore 改作"回合分"，先到目标回合数获胜。
  if (world.mode === 'lastman') {
    world.targetScore = LASTMAN_ROUNDS;  // 比分行显示"回合数 / 目标"
    world.roundOverTimer = 0;            // 回合结算展示倒计时（>0 时冻结、不复活）
    world.roundWinner = null;            // 上一回合赢家（客户端显示提示）
  }
...
function creditKill(world, killerId, victim) {
  ...
  // 占点/夺旗/最后存活：击杀只计个人战绩，不直接决定胜负。把刚加的队伍分回退
  // （teamScore 在这些模式里另作占点分/夺旗数/回合数，由各自的判定函数管理）。
  if (world.mode === 'koth' || world.mode === 'ctf' || world.mode === 'lastman') {
    world.teamScore[killer.team]--; return;
  }
...
function tickFighter(world, f) {
  ...
  if (!f.alive) {
    if (world.mode === 'lastman') return; // 最后存活：本回合内不复活（回合切换时统一重生）
    f.respawnTimer--;
...
function updateWorld(world, inputs) {
  ...
  // 最后存活：回合结算展示期，冻结全场，倒计时结束后重置下一回合
  if (world.mode === 'lastman' && world.roundOverTimer > 0) {
    world.roundOverTimer--;
    if (world.roundOverTimer === 0) startNextRound(world);
    return;
  }
  ...
  if (world.mode === 'lastman') checkRoundEnd(world); // 最后存活：某队全灭则本回合结束
}

// 最后存活：某队全员阵亡 → 对方赢一回合；先到目标回合数则整局结束
function checkRoundEnd(world) {
  if (world.roundOverTimer > 0) return; // 已在结算展示中
  let greenAlive = 0, redAlive = 0;
  for (const f of world.fighters) {
    if (!f.alive) continue;
    if (f.team === 'green') greenAlive++; else redAlive++;
  }
  let winner = null;
  if (greenAlive === 0 && redAlive > 0) winner = 'red';
  else if (redAlive === 0 && greenAlive > 0) winner = 'green';
  else if (greenAlive === 0 && redAlive === 0) winner = 'draw'; // 同归于尽：本回合算平，不计分
  if (!winner) return;

  if (winner !== 'draw') world.teamScore[winner]++;
  world.roundWinner = winner;
  world.roundOverTimer = ROUND_OVER_TICKS;
  world.soundEvents.push({ type: 'explode' }); // 借爆炸音烘托回合结束

  // 先到目标回合数 → 整局结束
  if (winner !== 'draw' && world.teamScore[winner] >= world.targetScore) {
    world.gameOver = true; world.winner = winner;
  }
}

// 开下一回合：全员满血原地复活（回到各自基地附近出生点）、清场、给开局倒计时
function startNextRound(world) {
  world.roundWinner = null;
  // 重新散开放置所有单位到各队出生点，满血复活
  const greenSpawns = spreadSpawns(world, world.greenBase, world.fighters.filter(f => f.team === 'green').length);
  const redSpawns = spreadSpawns(world, world.redBase, world.fighters.filter(f => f.team === 'red').length);
  let gi = 0, ri = 0;
  for (const f of world.fighters) {
    const s = f.team === 'green' ? greenSpawns[gi++] : redSpawns[ri++];
    if (s) { f.x = s.x; f.y = s.y; }
    f.alive = true; f.hp = f.maxHp; f.invuln = SPAWN_PROTECT;
    f.weapon = 'pistol'; f.ammo = Infinity; f.grenades = START_GRENADES;
    f.streak = 0; f.streakTimer = 0; f.respawnX = null; f.respawnY = null;
  }
  // 清掉残留的子弹/手雷/特效，干净开局
  world.bullets = []; world.grenades = []; world.effects = [];
  world.countdown = COUNTDOWN_TICKS; // 下一回合也走 3-2-1 倒计时
}
...
function snapshot(world) {
  ...
  return {
    ...
    roundOver: world.roundOverTimer || 0, roundWinner: world.roundWinner || null,
  };
}
...
function snapshotBinary(world) {
  ...
  // 先算总长度，精确分配
  let len = 1 + 1 + 1 + 1 + 1 + 2 + 2;  // ver + flags + winner + countdown + roundByte + greenScore + redScore
  ...
  // roundByte：bit0 = 是否回合结算展示中，bit1-2 = 回合赢家(0无/1绿/2红/3平)
  const rw = world.roundWinner === 'green' ? 1 : (world.roundWinner === 'red' ? 2 : (world.roundWinner === 'draw' ? 3 : 0));
  buf.writeUInt8(((world.roundOverTimer > 0) ? 1 : 0) | (rw << 1), o); o += 1;
